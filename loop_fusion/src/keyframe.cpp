...
/**
 * 构造关键帧
 * 1、提取滑窗中特征点对应的Brief描述子
 * 2、提取当前帧图像角点对应的Brief描述子
 * @param _time_stamp		时间戳
 * @param _index			关键帧idx
 * @param _vio_T_w_i		位姿Twi
 * @param _vio_R_w_i		位姿Twi
 * @param _image			图像
 * @param _point_3d			特征点世界坐标
 * @param _point_2d_uv		特征点像素坐标
 * @param _point_2d_norm	特征点归一化相机坐标
 * @param _point_id			特征点id
 * @param _sequence			
*/
...
/**
 * 描述子匹配，返回最佳匹配点、点向量
 * @param window_descriptor		描述子
 * @param descriptors_old		待匹配描述子集合
 * @param keypoints_old			待匹配特征点集合
 * @param keypoints_old_norm	待匹配特征点向量集合
 * @param best_match			output 最佳匹配点
 * @param best_match_norm		output 最佳匹配点向量
*/
...
/**
 * 计算匹配特征点
 * 当前帧特征点与闭环帧特征点进行匹配，通过计算描述子之间的距离，找到当前帧特征点描述子在闭环帧描述子集合中最近的那个，返回对应点
*/
...
/**
 * 3d-2d Pnp计算匹配闭环帧的位姿
 * @param matched_2d_old_norm	匹配闭环帧点归一化相机平面点
 * @param matched_3d			世界坐标点
 * @param status				output 匹配状态
 * @param PnP_T_old				output 闭环匹配帧的位姿
 * @param PnP_R_old				output 闭环匹配帧的位姿
*/
